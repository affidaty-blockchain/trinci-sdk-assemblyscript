...
function checkCustomRunFunction(node) {
    ...
    if (!node.signature.parameters || node.signature.parameters.length != 4)
        throw new Error(signatureErrorMessage);
    node.signature.parameters.forEach((param) => {
        if (utils.getName(param.type) != 'u32')
            throw new Error(signatureErrorMessage);
    });
}
function checkCustomAllocFunction(node) {
    ...
    if (utils.getName(node.signature.returnType) != 'u32')
        throw new Error(signatureErrorMessage);
    if (!node.signature.parameters || node.signature.parameters.length != 1)
        throw new Error(signatureErrorMessage);
    node.signature.parameters.forEach((param) => {
        if (utils.getName(param.type) != 'u32')
            throw new Error(signatureErrorMessage);
...
function checkCustomIsCallableFunction(node) {
    ...
    if (utils.getName(node.signature.returnType) != 'u8')
        throw new Error(signatureErrorMessage);
...
function parseTopLevelStatement(sourcePath, stmtStr) {
    ...
    if (!stmt)
        throw new Error(`Error parsing top level statement; Source path "${sourcePath}"; Statement string: ${stmtStr}`);
    return stmt;
}
