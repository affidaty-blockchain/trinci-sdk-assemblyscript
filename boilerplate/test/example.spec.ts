...
describe('Test all smart contract functionalities', () => {
    ...
    it('get_config method', async () => {
        ...
        expect(txResult.resultDecoded).toEqual({"max_mintable": 0, "owner": "", "total_minted": 0});
    });
    ...
    it('transfer method', async () => {
        ...
        expect(node.db.getAccountAssetPacked('AnotherAccount', '#TargetAccount')).toEqual(1000 - 100); // check if balance of AnotherAccount is equal to minted tokens - transfer tokens
        expect(node.db.getAccountAssetPacked('DestAccount', '#TargetAccount')).toEqual(100); // check if balance of AnotherAccount is equal to minted tokens - transfer tokens
        node.db.printAssets();  // use this tool to print in console the table of KVStore about any account such as #TargetAccount
    });
});
